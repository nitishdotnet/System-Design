...
public class BankAccount {
    ...
    public BankAccount(double initialBalance) {
        if (initialBalance < 0) throw new ArgumentException("Negative balance");
...
```csharp
class Animal {          // Parent
    void eat() { Console.WriteLine("Eating..."); }
}

class Dog : Animal {   // Child — Dog IS-A Animal
    void bark() { Console.WriteLine("Bark!"); }
}
...
public class EmployeeDiscount : DiscountStrategy {
...
public class TaxInvoice : Invoice {
    override
    public double getInvDiscount(double amount) {
        return amount - 200;
    }
...
class CryptoCoin : PaymentMethod {
    override
    void pay(double amount) {
...
class ShadyWallet : PaymentMethod {
    override
    void pay(double amount) {
...
public class Human : Workable, Eatable, Sleepable {
    public void work()  { /* works */ }
...
class Dog : Animal {
    void speak() { Console.WriteLine("Bark!"); }
}
...
class DogCat : Dog, Cat {   // COMPILATION ERROR in Java
...
class DogCat : Barkable, Meowable {
    override
...
```csharp
interface A {
    default void greet() { Console.WriteLine("Hello from A"); }
}

interface B {
    default void greet() { Console.WriteLine("Hello from B"); }
}
...
class Animal {
    ...
    void sleep() { Console.WriteLine(name + " is sleeping"); }
}

class Dog : Animal {
    void bark() { Console.WriteLine("Woof!"); }
...
class Tiger : Animal {
    void roar() { Console.WriteLine("ROAR!"); }
}
...
class BigCat : Animal {                          // ← introduced middle layer
    void roar() { Console.WriteLine("ROAR!"); }       // ← shared by big cats only
}

class Tiger   : BigCat { /* tiger-specific */ }
class Lion    : BigCat { /* lion-specific */ }
class Leopard : BigCat { /* leopard-specific */ }

class Dog : Animal { void bark() {} }            // Dog still doesn't get roar()
```
...
class Tiger : Animal : Roarable { public void roar() {...} }
class Lion  : Animal : Roarable { public void roar() {...} }
...
```csharp
class Animal {
    void eat() { Console.WriteLine(name + " eats grass"); }   // default
}
...
class Tiger : Animal {
    override
    void eat() { Console.WriteLine(name + " hunts and eats meat"); }   // tiger overrides
}
```
...
class Airplane : Flyable {   // Airplane REALIZES Flyable
    override
    public void fly() {
        Console.WriteLine("Flying with engines");
    }
}
...
```csharp
// Generalization (: a CLASS)
class Animal { ... }
class Dog : Animal { ... }            // Dog IS-A Animal — Generalization

// Realization (: an INTERFACE)
interface Flyable { void fly(); }
class Bird : Flyable { ... }       // Bird REALIZES Flyable — Realization
```
...
class Cook    : ICook    { public void cookFood()   { /* ... */ } }
class Waiter  : IWaiter  {
    public void takeOrder()   { /* ... */ }
    public void serveDishes() { /* ... */ }
}
class Cleaner : ICleaner { public void cleanTable() { /* ... */ } }
...
class ReportService : PaymentService {
...
class CreditCardPayment : IPaymentProcessor, IRefundProcessor {
    override public void pay()    { /* ... */ }
    override public void refund() { /* ... */ }
}

// Reporting service only generates reports
class ReportingService : IReportGenerator {
    override public void generateReport() { /* ... */ }
}
```
...
class XYZSQLConnection  : DBConnection {
...
class StripeAdapter   : PaymentGateway { /* calls Stripe SDK */ }
class PayPalAdapter   : PaymentGateway { /* calls PayPal SDK */ }
class FakePayment     : PaymentGateway { /* records calls in memory, for tests */ }
```
...
class JpaOrderRepository      : OrderRepository { /* uses JPA */ }
class MongoOrderRepository    : OrderRepository { /* uses MongoDB */ }
class InMemoryOrderRepository : OrderRepository { /* HashMap, for tests */ }
```
...
double total(List<Item> items,
             bool isCustomer, double customerDiscount,
             bool isVendor) {
    double subtotal = items.stream().mapToDouble(Item::price).sum();
...
class EvenCheckerImpl : EvenChecker {
    override
...
public class Holder {
    ...
    static {
        Console.WriteLine("Holder being initialized");
    }
}
```
...
public class Main {
    public static void main(string[] args) {
        ...
        if (db1 == db2) {
            Console.WriteLine("Same instance ✓");   // always true
        }
    }
}
```
...
```csharp
button.addClickListener(() -> Console.WriteLine("clicked"));   // 1 line
```
...
Consumer<string> c        = msg -> Console.WriteLine(msg);
c.accept("hi");                                          // prints "hi"

Function<string, int> f = str -> str.length();
int len                   = f.apply("abc");              // 3

Predicate<int> p      = n -> n > 0;
bool isPositive        = p.test(5);                   // true
```
...
Consumer<string> c1 = x -> Console.WriteLine(x);   // lambda
...
Supplier<int> len = greeting::length;   // bound to THIS specific "Hello"
...
```csharp
Function<string, int> len = string::length;   // unbound — works on ANY string
len.apply("Hello");  // 5
len.apply("Hi");     // 2
len.apply("Java");   // 4
```
...
```csharp
int::parseInt    →  needs 1 input (a string)        →  returns int
...
list::add            →  needs 1 input (the element)     →  returns bool (often ignored)
...
calculator.compute(10, 20, int::sum);   // strategy: sum
...
class CreditCardPayment : Payment {
    override
    public void process() {
        Console.WriteLine("Processing CC Payment");
    }
}
...
class PaymentFactory {
    ...
    public static Payment getPayType(string type) {
        ...
        if (creator == null) throw new ArgumentException("Unknown: " + type);
...
public class PaymentService {
    ...
    public void process(string type) {
        ...
        if (p == null) throw new ArgumentException("Unknown: " + type);
        p.process();
    }
}
...
class AWSStorage : Storage {
    ...
    public void upload(string file) {
        Console.WriteLine("Uploading to AWS S3: " + file);
        // AWSSDK.connect(file);
    }
}
...
class AWSDB : Database {
    override
    public void connect() {
        Console.WriteLine("Connecting to AWS RDS");
    }
}
```
...
class AzureStorage : Storage {
    override
    public void upload(string file) {
        Console.WriteLine("Uploading to Azure Blob: " + file);
    }
}
...
class Pizza {
    Pizza(string size) { ... }
    Pizza(string size, bool cheese) { ... }
    Pizza(string size, bool cheese, bool pepperoni) { ... }
    Pizza(string size, bool cheese, bool pepperoni, bool olives) { ... }
    Pizza(string size, bool cheese, bool pepperoni, bool olives,
          bool mushrooms, bool basil, string crust, string sauce) { ... }
    // ... and on, and on
}
...
```csharp
class SQLQuery {
    SQLQuery(string select, string from, string where, string orderBy, int limit) {
        // ...
    }
}
```
...
public class RazorpayAdapter : PaymentProcessor {
    ...
    override
    public void pay(double amount) {           // : the STANDARD interface
...
public class InventoryService {
    public bool checkStock(string item) {
        Console.WriteLine("Checking inventory for " + item);
        return true;
    }
}
...
public class OrderFacade {
    ...
    public void placeOrder(string item, double amount) {
        try {
            ...
            Console.WriteLine("Order failed: " + e.getMessage());
        }
    }
}
```
...
class CircleWithRedBorder : Circle { void draw() { /* draw + add border */ } }
class CircleWithShadow : Circle { void draw() { /* draw + add shadow */ } }
class CircleWithRedBorderAndShadow : Circle { /* both! */ }
class RectangleWithRedBorder : Rectangle { ... }
class RectangleWithShadow : Rectangle { ... }
...
public class RedBorderDecorator : Shape {
    ...
    private void setRedBorder() { Console.WriteLine("Adding Red Border"); }
}

public class ShadowDecorator : Shape {
    ...
    private void addShadow() { Console.WriteLine("Adding Shadow"); }
}
```
...
public class Underline : TextDecorator {
...
public class Color : TextDecorator {
...
public class FilterInputStream : InputStream {
    ...
    override public bool markSupported()         { return in.markSupported(); } // pass-through
}
```
...
public class ShapeCache {
    ...
    public static Shape getCircle(string color) {
        ...
        if (circle == null) {                                  // first time?
            ...
            Console.WriteLine("Creating a new " + color + " Circle");
        }
...
1. **`Integer.valueOf(int)`** — Java pre-creates Integer objects for `-128` to `127` and reuses them. So:
   ```csharp
   int.valueOf(100) == int.valueOf(100)   // true — SAME shared object
   int.valueOf(200) == int.valueOf(200)   // false — outside the cache range, fresh objects
...
public class PaymentService {
    public void processPayment(string paymentType, int amount) {
        if (paymentType.equals("UPI")) {
            ...
            Console.WriteLine("Pay " + amount + " using UPI");
        } else if (paymentType.equals("CARD")) {
            // Card logic — call Visa/Mastercard APIs
            Console.WriteLine("Pay " + amount + " using Card");
        } else if (paymentType.equals("NETBANKING")) {
            // NetBanking logic
            Console.WriteLine("Pay " + amount + " using NetBanking");
        } else {
...
```csharp
public class UPIPayment : PaymentStrategy {
    override
...
public class NetBankingPayment : PaymentStrategy {
    override
    public void pay(int amount) {
        Console.WriteLine("Pay " + amount + " using NetBanking");
    }
...
service.setPaymentStrategy(amount -> Console.WriteLine("UPI: " + amount));
service.processPayment(500);

service.setPaymentStrategy(amount -> Console.WriteLine("Card: " + amount));
...
public class OrderService {
    ...
    public void updateOrderStatus(string orderId, string status) {
        Console.WriteLine("Order status updated to status: " + status);
...
```csharp
public class EmailService : OrderObserver {
    override
    public void update(string orderId, string status) {
        Console.WriteLine("Email sent for order " + orderId + " with status " + status);
    }
}

public class WarehouseService : OrderObserver {
    override
    public void update(string orderId, string status) {
        Console.WriteLine("Warehouse update for order " + orderId + " " + status);
    }
}

public class AnalyticsService : OrderObserver {
    override
    public void update(string orderId, string status) {
        Console.WriteLine("Analytics track for order " + orderId);
    }
}
```
...
```csharp
public class OneTimeSmsObserver : OrderObserver {
...
public void approve(ExpenseReport report) {
    if (report.getAmount() <= 10_000) {
        ...
        Console.WriteLine("CFO approved");
    }
}
```
...
public class Manager : Approver {
    ...
    public void approve(ExpenseReport report) {
        if (report.getAmount() <= 10_000) {
            Console.WriteLine("Manager approved: " + report.getPurpose());
        } else if (next != null) {
...
public class Director : Approver {
    ...
    public void approve(ExpenseReport report) {
        if (report.getAmount() <= 50_000) {
            Console.WriteLine("Director approved: " + report.getPurpose());
        } else if (next != null) {
...
public class VP : Approver {
    ...
    public void approve(ExpenseReport report) {
        if (report.getAmount() <= 2_00_000) {
            Console.WriteLine("VP approved: " + report.getPurpose());
        } else if (next != null) {
...
public class CFO : Approver {
    override
    public void approve(ExpenseReport report) {
        Console.WriteLine("CFO will take final action: " + report.getPurpose());
...
```csharp
public class LoggingHandler : Approver {
    public void approve(ExpenseReport report) {
        Console.WriteLine("LOG: incoming " + report);   // always do my work
        if (next != null) {
...
```csharp
public class RejectAllFallback : Approver {
    override
    public void approve(ExpenseReport report) {
        Console.WriteLine("REJECTED — exceeds all approval limits: " + report.getPurpose());
...
public class ChainDemo {
    public static void main(string[] args) {
        ...
        try {
            ...
            Console.WriteLine("Caught: " + e.getMessage());
...
public abstract class AbstractAPIHandler {
    ...
    protected void auditLog(string request, string response) {
        Console.WriteLine("Log Req & Response");
    }
...
```csharp
public class PaymentAPIHandler : AbstractAPIHandler {

    override
    protected void validate(string request) {
        Console.WriteLine("Validation of request");
    }
...
```csharp
class PaymentAPIHandler : AbstractAPIHandler {
    override
...
class FraudCheckHandler : AbstractAPIHandler {
    override
    protected void validate(string req) { /* ... */ }
...
```csharp
public class BadPaymentHandler : AbstractAPIHandler {
    override
...
```csharp
class AuditFirstHandler : AbstractAPIHandler {
    override
...
```csharp
class IncompleteHandler : AbstractAPIHandler {
...
class SloppyHandler : AbstractAPIHandler {
    override
    protected void validate(string req) {
        if (req == null) {
...
```csharp
// You want:
class PaymentHandler : AbstractAPIHandler, AbstractMonitored { ... }   // ❌ Java doesn't allow this
```
